# Skill Mapping Reference

> Master index mapping the skills library to the 13 Zeoel sub-agents.
> Each agent gets curated, relevant skills.
> ⭐ marks the agent's primary/most-used skills.

---

## Orchestration

### Gohar (CEO / Orchestrator)
- `zeoel` ⭐
- `zeoel-saas-architect` ⭐
- `zeoel-codebase-knowledge`
- `plan-orchestrate`
- `best-practice`
- `coding-standards`
- `architecture-decision-records`
- `git-workflow`
- `project-flow-ops`
- `repo-scan`
- `codebase-onboarding`
- `strategic-compact`

---

## Design & Visual

### Mahdi (Product Designer — SEO-First)
- `frontend-design` ⭐
- `seo` ⭐
- `frontend-design-direction`
- `design-system`
- `design-auditor`
- `mobile-app-design`
- `mobile-app-ui-design`
- `make-interfaces-feel-better`
- `canvas-design`
- `brand-guidelines`
- `brand-voice`
- `nothing-design`
- `liquid-glass-design`
- `click-path-audit`
- `accessibility`

### Mustafa (Visual/Art Director — 3D, GSAP, Premium Styling)

**Core Design**
- `frontend-design` ⭐
- `ui-ux-pro-max` ⭐
- `modern-web-design` ⭐
- `theme-factory`
- `design-system`
- `ui-demo`
- `brand-guidelines`
- `liquid-glass-design`
- `nothing-design`
- `canvas-design`
- `frontend-slides`

**3D & WebGL**
- `threejs-webgl` ⭐
- `react-three-fiber`
- `lightweight-3d-effects`
- `spline-interactive`
- `babylonjs-engine`
- `pixijs-2d`

**Animation & Motion**
- `gsap-scrolltrigger` ⭐
- `motion-framer` ⭐
- `animejs`
- `motion-foundations`
- `motion-advanced`
- `motion-patterns`
- `motion-ui`
- `scroll-reveal-libraries`
- `locomotive-scroll`
- `barba-js`
- `lottie-animations`
- `rive-interactive`
- `react-spring-physics`
- `animated-component-libraries`

---

## Engineering

### Karar (Senior Frontend Engineer — Next.js, shadcn, 3D, GSAP, SEO)

**Core Framework**
- `nextjs-turbopack` ⭐
- `frontend-patterns`
- `frontend-design` ⭐
- `seo` ⭐
- `vite-patterns`
- `error-handling`

**Premium UI & Design System**
- `ui-ux-pro-max` ⭐
- `modern-web-design`
- `theme-factory`
- `design-system`
- `ui-demo`
- `liquid-glass-design`
- `nothing-design`
- `canvas-design`

**3D & WebGL**
- `threejs-webgl` ⭐
- `react-three-fiber`
- `lightweight-3d-effects`
- `spline-interactive`

**Animation & Motion**
- `gsap-scrolltrigger` ⭐
- `motion-framer`
- `animejs`
- `motion-foundations`
- `motion-advanced`
- `motion-patterns`
- `motion-ui`
- `scroll-reveal-libraries`
- `locomotive-scroll`
- `barba-js`
- `lottie-animations`

**Testing**
- `e2e-testing`
- `webapp-testing`
- `tdd-workflow`

### Tariq (Backend Engineer — Laravel & PostgreSQL SaaS)
- `laravel-patterns` ⭐
- `laravel-security` ⭐
- `laravel-tdd`
- `laravel-verification`
- `laravel-plugin-discovery`
- `postgres-patterns` ⭐
- `mysql-patterns`
- `redis-patterns`
- `prisma-patterns`
- `api-design` ⭐
- `api-connector-builder`
- `database-migrations`
- `backend-patterns`
- `hexagonal-architecture`
- `nestjs-patterns`
- `docker-patterns`
- `security-review` ⭐
- `gateguard`
- `production-audit`

---

## Specialized SaaS Agents

### Zara (Content & SEO Specialist)
- `seo` ⭐
- `brand-voice`
- `brand-guidelines`
- `frontend-design`
- `click-path-audit`
- `accessibility`
- `product-lens`
- `product-capability`

### Hassan (Mobile Developer — Flutter)
- `dart-flutter-patterns` ⭐
- `flutter-dart-code-review` ⭐
- `mobile-app-design` ⭐
- `mobile-app-ui-design`
- `api-connector-builder`
- `frontend-patterns`
- `e2e-testing`
- `tdd-workflow`
- `error-handling`
- `motion-foundations`

### Fatima (Data & ML Engineer)
- `postgres-patterns` ⭐
- `python-patterns` ⭐
- `python-testing`
- `mle-workflow` ⭐
- `pytorch-patterns`
- `database-migrations`
- `mysql-patterns`
- `benchmark`
- `data-scraper-agent`
- `production-audit`
- `redis-patterns`
- `deep-research`
- `dashboard-builder`
- `recsys-pipeline-architect`

---

## Support Agents

### Gohar — Python (Python & Automation)
- `python-patterns` ⭐
- `python-testing` ⭐
- `fastapi-patterns`
- `django-patterns`
- `django-security`
- `django-tdd`
- `django-celery`
- `mle-workflow` ⭐
- `pytorch-patterns`
- `data-scraper-agent`
- `automation-audit-ops`
- `benchmark`
- `deep-research`
- `cost-aware-llm-pipeline`

### Muhammad (QA Engineer)
- `e2e-testing` ⭐
- `webapp-testing` ⭐
- `tdd-workflow`
- `verification-loop`
- `browser-qa`
- `ai-regression-testing`
- `laravel-tdd`
- `laravel-verification`
- `flutter-dart-code-review`
- `production-audit`
- `benchmark`
- `security-scan`
- `plankton-code-quality`
- `click-path-audit`
- `coding-standards`

### Sajjad (Debugger & Performance Specialist)
- `agent-introspection-debugging` ⭐
- `error-handling` ⭐
- `verification-loop`
- `browser-qa`
- `production-audit`
- `benchmark` ⭐
- `postgres-patterns`
- `redis-patterns`
- `plankton-code-quality`
- `coding-standards`
- `security-scan`
- `best-practice`
- `architecture-decision-records`
- `laravel-verification`
- `laravel-tdd`
- `webapp-testing`
- `e2e-testing`
- `flutter-dart-code-review`

### Baqir (Documentation & API Specialist)
- `zeoel-codebase-knowledge` ⭐
- `codebase-onboarding` ⭐
- `api-design` ⭐
- `api-connector-builder`
- `coding-standards`
- `architecture-decision-records`
- `article-writing`
- `content-engine`
- `changelog-generator`
- `repo-scan`
- `brand-voice`
- `product-capability`
- `product-lens`

### Ali (DevOps & Security Engineer)
- `deployment-patterns` ⭐
- `docker-patterns` ⭐
- `github-ops`
- `git-workflow`
- `security-review` ⭐
- `security-scan`
- `security-bounty-hunter`
- `zeoel-security` ⭐
- `production-audit`
- `development-workflows`
- `dmux-workflows`
- `repo-scan`
- `codebase-onboarding`
- `zeoel-codebase-knowledge`
- `canary-watch`
- `gateguard`
