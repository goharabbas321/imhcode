...
Do NOT include any preamble, explanation, or commentary outside the Markdown document.`;
  ...
  if (fileCreatedOrModified) {
    ...
    if (llmOutput && llmOutput.trim().length > 200) {
      ...
      console.log('\n⚠️  LLM brainstorming generation failed or returned empty output.');
      const answer = await askQuestion('Would you like to fall back to the static brainstorming template? [y/N]: ');
      if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
        ...
        console.log('\n❌ Aborted. Please check your model limits and run "imhcode init" again.');
        process.exit(1);
      }
    }
  }
...
async function generateSprintPlans(cwd, userPrompt, brainstormContent, config) {
  ...
  if (llmSprintResult) {
    ...
    console.log('\n⚠️  LLM sprint plans generation failed or returned empty output.');
    const answer = await askQuestion('Would you like to fall back to the static sprint plans template? [y/N]: ');
    if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
      ...
      console.log('\n❌ Aborted. Please check your model limits and run "imhcode plan" again.');
      process.exit(1);
    }
  }
